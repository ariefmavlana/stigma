...
def create_crew_callback(celery_task, start_time, current_logs):
    ...
    def step_callback(step_output):
        # Extract meaningful info from the step output
        # CrewAI step_output can be AgentAction, ToolResult, or AgentFinish
        message = "Agent is active..."
        
        # Try to get the "thought" or action description
        if hasattr(step_output, 'thought'):
            message = step_output.thought
        elif hasattr(step_output, 'tool'):
            message = f"Using tool: {step_output.tool} with input {getattr(step_output, 'tool_input', '')}"
        
        # Limit message length
        message = (message[:120] + '...') if len(message) > 120 else message
        
        # Append to logs and keep last 10
        ...
        celery_task.update_state(
            state='PROGRESS',
            meta={
                'progress': 40, # Generic 'in-progress' value
                'message': message,
...
def generate_ai_post_task(self, topic, target_audience, tone, category_id, user_id, start_time):
    """
    ...
    try:
        ...
        self.update_state(state='PROGRESS', meta={
            ...
            'start_time': start_time,
            'logs': logs + ["Task complete. Saving to database..."]
        })
